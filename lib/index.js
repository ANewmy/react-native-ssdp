...
SSDP.prototype._init = function(opts) {
    ...
    if (typeof opts.location === 'function') {
        ...
        NetworkInfo.getIPAddress(ip => {
            this._location = opts.location || 'http://' + ip + ':' + 10293 + '/upnp/desc.html';
        });
    }
...
SSDP.prototype._start = function(port, host, cb) {
    ...
    this.sock.addListener('listening', function onSocketListening() {
        ...
        function addMembership() {
            try {
                // The following line prevents Android 7 from closing the socket
                ...
                if (e.code === 'ENODEV' || e.code === 'EADDRNOTAVAIL') {
                    self._logger(
                        'No interface present to add multicast group membership. Scheduling a retry. %s',
                        e.message
                    );
...
SSDP.prototype._parseMessage = function(msg, rinfo) {
    //msg = msg.toString();
    msg = new Buffer(msg, 'base64').toString();
